...
def batch_view(request, **kwargs):
    ...
    if user.groups.filter(name__in=['Koordynatorzy domu']).exists() or \
        ...
        return render(request, template_name, {'batch_details': batch_details})
    else:
        return redirect('homepanel:home_coordinator_login')
...
def participant_sign(request, **kwargs):
    ...
    for b in future_batches:
        ...
        men_wheelchair = BatchParticipant.objects.filter(batch=b, participant__sex='M',
                                                         participant__wheelchair=True).count()
...
def participant_details(request, **kwargs):
    ...
    if user.groups.filter(name__in=['Koordynatorzy domu']).exists() or \
            user.groups.filter(name='Koordynatorzy zapisów').exists():
        participant = Participant.objects.filter(id=kwargs['pk']).first()
        return render(request, template_name, {'participant': participant})
    else:
        return redirect('homepanel:home_coordinator_login')
...
def add_booking_2(request, **kwargs):
    ...
    if request.user.groups.filter(name__in=['Koordynatorzy domu']).exists():
        if request.POST and form.is_valid():
            ...
            fp = open('dj/static/images/small-icons.png', 'rb')
            image = MIMEImage(fp.read())
            fp.close()

            fp = open('dj/static/images/orione-name.png', 'rb')
        ...
        with open('test - podopieczni.csv', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                participant = Participant.objects.create(
                    ...
                    not_need_help=not_need_help,
                    wheelchair=wheelchair,
                    disabled_hands=disabled_hands,
                    crutches=crutches,
                    disabled_sight=disabled_sight,
                    disabled_hearing=disabled_hearing,
                    epilepsy=epilepsy
                )

                batch = Batch.objects.filter(id=1).first()     # 5
                if row[17] == 'Sylwester':
                    batch = Batch.objects.filter(id=2).first()  # s
                elif row[17] == 'Brańszczyk1':
                    batch = Batch.objects.filter(id=3).first()  # 1
                elif row[17] == 'Brańszczyk2':
                    batch = Batch.objects.filter(id=4).first()  # 2
                elif row[17] == 'Brańszczyk3':
                    batch = Batch.objects.filter(id=5).first()  # 3
                elif row[17] == 'Brańszczyk4':
                    batch = Batch.objects.filter(id=6).first()  # 4
