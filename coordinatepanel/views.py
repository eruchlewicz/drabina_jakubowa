...
def batch_volunteer_edit(request, **kwargs):
    ...
    if request.user.groups.filter(name__in=['Koordynatorzy']).exists():
        if request.POST and form.is_valid():
            ...
            if batch_volunteer.batch_begin_date < batch.begin_date or batch_volunteer.batch_begin_date > batch.end_date:
                batch_volunteer.batch_begin_date = batch.begin_date
            if batch_volunteer.batch_end_date > batch.end_date or batch_volunteer.batch_end_date < batch.begin_date:
...
class ParticipantsReportPDFView(PDFTemplateResponseMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        return super(ParticipantsReportPDFView, self).get_context_data(
            ...
            title='Raport',
            date=now,
            batchparticipants=BatchParticipant.objects.filter(batch=self.kwargs["pk"]).order_by('participant__sex',
...
def statistics(request):
    ...
    if user.groups.filter(name__in=['Koordynatorzy']).exists():
        ...
        queryset = Batch.objects.filter(begin_date__year=now.year) | Batch.objects.filter(end_date__year=now.year)
