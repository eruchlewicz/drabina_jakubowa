...
def index2(request):
    event_posts = EventPost.objects.filter(begin_date__gte=now).order_by('begin_date')
    return render(request, 'dj/home2.html', {'now': now, 'event_posts': event_posts})
        ...
        if user_form.is_valid() and volunteer_form.is_valid() and volunteer_batch_form.is_valid():
            ...
            if password == password_confirmation and data_processing_agreement is True:
                ...
                try:
                    ...
                    to_email = volunteer.email_address
                    ...
                    image = generate_image('dj_sign.jpg', str(batch), batch.begin_date, volunteer.id)
                    out_buf = io.BytesIO()
                    image.save(out_buf, format="JPEG")
                    img = MIMEImage(out_buf.getvalue(), name='Potwierdzenie.jpg')
                    out_buf.close()
                    msg.attach(img)

                    image = generate_image('dj_sign2.jpg', str(batch), batch.begin_date, volunteer.id)
                    out_buf = io.BytesIO()
                    image.save(out_buf, format="JPEG")
                    img = MIMEImage(out_buf.getvalue(), name='Potwierdzenie.jpg')
                    out_buf.close()
                    ...
                    fp = open('/home/drabinajakuba/websites/engineeringwork/drabinajakubowa/static/images/logo.png', 'rb')
            ...
            if user_form.is_valid() and volunteer_form.is_valid() and volunteer_event_form.is_valid():
                ...
                if password == password_confirmation and data_processing_agreement is True:
                    ...
                    if event is not None:
                        ...
                        volunteer_event.payment_id = "#wy" + str(event.id) + "w" + str(volunteer.id)
...
def batches_view(request):
    ...
    form.fields["batch"].queryset = (Batch.objects.filter(begin_date__year=now.year, end_date__gte=now,
                                                          id__in=batches, is_closed=False) |
                                     Batch.objects.filter(begin_date__year=now.year+1, id__in=batches,
                                                          is_closed=False))\
    ...
    if volunteer.mother_first_name is None or volunteer.mother_surname is None or volunteer.father_first_name is None:
...
def user_account_view(request):
    ...
    volunteer = Volunteer.objects.filter(user=request.user).first()
    if volunteer.mother_first_name is None or volunteer.mother_surname is None or volunteer.father_first_name is None:
        message = 'Proszę uzupełnić brakujące dane'
        return render(request, template_name, {'user_details': volunteer, 'message': message})
    else:
...
def events_view(request):
    ...
    form.fields["event"].queryset = (Event.objects.filter(begin_date__year=now.year, begin_date__gte=now,
                                                          id__in=events, is_closed=False) |
                                     Event.objects.filter(begin_date__year=now.year+1, begin_date__gte=now,
                                                          id__in=events, is_closed=False) |
                                     Event.objects.filter(end_date__gte=now, id__in=events, is_closed=False))\
        .distinct()\
        .order_by('begin_date')
    if request.method == 'POST':
        if form.is_valid():
            ...
            if user_exists_in_event == 0:
                ...
                event_volunteer.payment_id = "#wy" + str(event.id) + "w" + str(volunteer.id)
...
def event_details(request, **kwargs):
    ...
    event_participants = EventParticipant.objects.filter(event=event, volunteer=volunteer)
            ...
            if form.is_valid() and form2.is_valid():
                ...
                if music_training is not None:
                    ...
                    if person_exists_in_training == 0:
                        ...
                        training.payment_id = "#wm" + str(training.id) + "i" + str(person.id)
    ...
    if request.user.is_authenticated:
        ...
        form2.fields["retreat_or_music_training"].queryset = \
            (RetreatOrMusicTraining.objects.filter(begin_date__year=now.year, begin_date__gte=now, type='W',
                                                   is_closed=False) |
             RetreatOrMusicTraining.objects.filter(begin_date__year=now.year+1, type='W', is_closed=False))\
            .distinct().order_by('begin_date')
        if request.method == 'POST':
            if form.is_valid() and form2.is_valid():
                music_training = form2.cleaned_data['retreat_or_music_training']
                if music_training is not None:
                    ...
                    if person_exists_in_training == 0:
                        ...
                        training.person = person
                        volunteer = Volunteer.objects.filter(pesel=person.pesel).first()
                        training.payment_id = "#wa" + str(training.id) + "i" + str(person.id)
                        if volunteer is not None:
                            training.volunteer = volunteer
            ...
            form2.fields["retreat_or_music_training"].queryset = \
                (RetreatOrMusicTraining.objects.filter(begin_date__year=now.year, begin_date__gte=now, type='W',
                                                       is_closed=False) |
                 RetreatOrMusicTraining.objects.filter(begin_date__year=now.year+1, type='W',
                                                       is_closed=False))\
                .distinct().order_by('begin_date')
...
def music_training_view(request):
    ...
    form.fields["retreat_or_music_training"].queryset = \
        (RetreatOrMusicTraining.objects.filter(begin_date__year=now.year, begin_date__gte=now, type='M',
                                               id__in=trainings, is_closed=False) |
         RetreatOrMusicTraining.objects.filter(begin_date__year=now.year+1, type='M', id__in=trainings,
                                               is_closed=False))\
        .distinct().order_by('begin_date')
    if request.method == 'POST':
        if form.is_valid():
        ...
        form.fields["retreat_or_music_training"].queryset = \
            (RetreatOrMusicTraining.objects.filter(begin_date__year=now.year, begin_date__gte=now, type='M',
                                                   id__in=trainings, is_closed=False) |
             RetreatOrMusicTraining.objects.filter(begin_date__year=now.year+1, type='W', id__in=trainings,
                                                   is_closed=False)) \
            .distinct().order_by('begin_date')
                    ...
                    form.fields["retreat_or_music_training"].queryset = \
                        (RetreatOrMusicTraining.objects.filter(begin_date__year=now.year, begin_date__gte=now, type='W',
                         ...
                         RetreatOrMusicTraining.objects.filter(begin_date__year=now.year+1, begin_date__gte=now,
                                                               type='W', id__in=trainings, is_closed=False)) \
                        .distinct().order_by('begin_date')
            ...
            if form.is_valid() and form2.is_valid():
                ...
                if music_training is not None:
                    ...
                    if person_exists_in_training == 0:
                        ...
                        training = form2.save()
                        training.person = person
                        training.payment_id = "#r" + str(training.id) + "i" + str(person.id)
                        volunteer = Volunteer.objects.filter(pesel=person.pesel).first()
                        if volunteer is not None:
                            training.volunteer = volunteer
        ...
        if form.is_valid():
            ...
            if user.count() > 0:
                ...
                for attempt in range(NUMBER_OF_RETRIES):
                    ...
                    raise Exception('Max number of retries for sending email')
...
def generate_image(file_name, batch, date, number):
    image = Image.open('/home/drabinajakuba/websites/engineeringwork/drabinajakubowa/static/images/' + file_name)
    draw = ImageDraw.Draw(image)
    (W, H) = image.size
    font = ImageFont.truetype('/home/drabinajakuba/websites/engineeringwork/drabinajakubowa/static/fonts/Cuprum-Bold.ttf', size=34)
    w, h = draw.textsize(batch.upper(), font=font)
    (x, y) = ((W - w) / 2, 390)
    color = 'rgb(255, 255, 255)'
    draw.text((x, y), batch.upper(), fill=color, font=font)
    font = ImageFont.truetype('/home/drabinajakuba/websites/engineeringwork/drabinajakubowa/static/fonts/Cuprum-Bold.ttf', size=30)
    w, h = draw.textsize("start " + str(date.day).zfill(2) + "." + str(date.month).zfill(2), font=font)
    (x, y) = ((W - w) / 2, 430)
    color = 'rgb(255, 255, 255)'
    draw.text((x, y), "start " + str(date.day).zfill(2) + "." + str(date.month).zfill(2), fill=color, font=font)
    font = ImageFont.truetype('/home/drabinajakuba/websites/engineeringwork/drabinajakubowa/static/fonts/Cuprum-Bold.ttf', size=30)
    w, h = draw.textsize("mój nr: " + str(number).zfill(3), font=font)
    (x, y) = ((W - w) / 2, 460)
    color = 'rgb(255, 255, 255)'
    draw.text((x, y), "mój nr: " + str(number).zfill(3), fill=color, font=font)
    return image


@login_required
def get_confirmation_images(request, **kwargs):
    template_name = 'dj/confirmation_images.html'
    batch = Batch.objects.filter(id=kwargs['batch']).first()
    volunteer_id = kwargs['vol'].zfill(3)
    image_1 = generate_image('dj_sign.jpg', str(batch), batch.begin_date, kwargs['vol'])
    img_buffer_1 = io.BytesIO()
    image_1.save(img_buffer_1, 'PNG')
    byte_array_1 = base64.b64encode(img_buffer_1.getvalue())
    image_2 = generate_image('dj_sign2.jpg', str(batch), batch.begin_date, kwargs['vol'])
    img_buffer_2 = io.BytesIO()
    image_2.save(img_buffer_2, 'PNG')
    byte_array_2 = base64.b64encode(img_buffer_2.getvalue())
    return render(request, template_name, {'batch': batch, 'volunteer_id': volunteer_id,
                                           'img_byte_array_1': byte_array_1, 'img_byte_array_2': byte_array_2})
